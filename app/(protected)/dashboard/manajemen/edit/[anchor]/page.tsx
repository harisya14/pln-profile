...
interface ManajemenSectionFormInput {
  title: string;
  anchor: string; // Anchor akan digunakan sebagai ID untuk edit
  orderIndex: number;
  assistant: {
    name: string;
    jabatan: string;
  ...
  containers: PersonFormData[][]; // Array of arrays of PersonData
}

export default function EditManajemenPage() {
  const router = useRouter();
  const params = useParams();
  const currentAnchor = params.anchor as string; // Anchor dari URL
  ...
  // --- useEffect untuk mengambil data manajemen yang akan diedit ---
  useEffect(() => {
    async function fetchManajemenData() {
      try {
        ...
        const data: ManajemenSectionFormInput = await res.json(); // API GET single mengembalikan objek tunggal

        // Setel form data dengan data yang diambil
        setFormData({
          title: data.title,
          anchor: data.anchor,
        ...
        toast({
          title: "Error",
          description: error.message || "Gagal memuat data manajemen.",
          variant: "destructive",
        });
        router.push("/dashboard/manajemen"); // Redirect jika gagal memuat
      } finally {
        setLoading(false);
      }
    }

    if (currentAnchor) { // Pastikan anchor tersedia dari params
      fetchManajemenData();
    }
  }, [currentAnchor, router]);
  // --- Akhir useEffect ---
  ...
  // Handler submit form (untuk update)
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000";
      // Menggunakan method PUT dan anchor dari URL
      const res = await fetch(`${baseUrl}/api/manajemen?anchor=${currentAnchor}`, {
        method: "PUT",
        headers: {
      ...
      if (res.ok) {
        toast({
          title: "Sukses",
          description: `Struktur manajemen "${formData.title}" berhasil diperbarui.`,
        });
        router.push("/dashboard/manajemen"); // Redirect ke halaman daftar manajemen
        router.refresh();
      } else {
        const errorData = await res.json();
        throw new Error(errorData.error || "Gagal memperbarui struktur manajemen.");
      }
    } catch (error: any) {
      console.error("Error updating manajemen structure:", error);
  ...
  if (loading) {
    return (
      <DashboardLayout>
        <div className="p-4 sm:p-6 text-center text-gray-600">Memuat data manajemen...</div>
      </DashboardLayout>
    );
  }
  ...
  if (formError) {
    return (
      <DashboardLayout>
        <div className="p-4 sm:p-6 text-center text-red-600">Error: {formError}</div>
      </DashboardLayout>
    );
  }
      ...
      <div className="p-4 sm:p-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800">
            Edit Struktur Manajemen: {formData.title}
          </h1>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4 border-b pb-6">
              ...
              <div>
                <Label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-1">Judul Bagian</Label>
                <Input
                  id="title"
                  name="title"
                  type="text"
              ...
              <div>
                <Label htmlFor="anchor" className="block text-sm font-medium text-gray-700 mb-1">Anchor (Tidak dapat diubah)</Label>
                <Input
                  id="anchor"
                  name="anchor"
                  type="text"
                  ...
                  onChange={handleInputChange}
                  disabled // Anchor tidak boleh diubah setelah dibuat
                  placeholder="anchor-unik"
                  required
                />
                <p className="mt-1 text-xs text-gray-500">Anchor digunakan sebagai ID unik dan tidak dapat diubah.</p>
              </div>
              <div>
            ...
            <Button
              ...
              disabled={submitting}
            >
              {submitting ? "Memperbarui Struktur..." : "Perbarui Struktur Manajemen"}
            </Button>
          </form>
        </div>
      </div>
